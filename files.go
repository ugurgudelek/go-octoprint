...
func (cmd *FileRequest) Do(c *Client) (*FileInformation, error) {
	uri := fmt.Sprintf("%s/%s/%s?force=true&recursive=%t", URIFiles,
...
func (cmd *FilesRequest) Do(c *Client) (*FilesResponse, error) {
	uri := fmt.Sprintf("%s?force=true&recursive=%t", URIFiles, cmd.Recursive)
	if cmd.Location != "" {
		uri = fmt.Sprintf("%s/%s?force=true&recursive=%t", URIFiles, cmd.Location, cmd.Recursive)
	}
